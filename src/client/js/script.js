!(() => {
			...
			switch (type) {
				case 'vk': {
					return createSvgIcon({
						...
						addNameSpace: ['svg', 'path'],
						addClass: ['contacts-list__link-icon'],
					});
				}
				case 'fb': {
					return createSvgIcon({
	...
	function createContactList(node, contactsArray) {
		...
		for (const contact of contactsArray) {
			...
			type === 'phone' ? link.href = `tel:${value}` : type === 'mail' ? link.href = `mailto:${value}` : link.href = value;
			...
			if (list.childElementCount > 3 && !list.querySelector('.contacts-list__btn') && contactsArray.length > 5) {
				const item = document.createElement('li');
	...
	function createContactsSection(contactsList = []) {
		function createCustomSelect(contact, {initalPhoneMask, contactType}) {
			...
			function selectChoose() {
				...
				const currentInput = this.closest('.contact-form__field').querySelector('.contact-form__input');
			...
			let phoneMask = contactType === 'phone' ? IMask(initalPhoneMask, {
				mask: "+{7}(000)000-00-00",
				lazy: false,
			}) : {};
		...
		function renderContactField(contact = {}) {
			...
			field.append(createCustomSelect(contact, {initalPhoneMask: input, contactType: contact.type || 'phone'}));
	...
	function extractContactsInfo(node) {
		...
		node.forEach(element => {
			...
			if (element.querySelector('.contact-select').dataset.contact === 'phone') {
				mask = IMask(element.querySelector('.contact-form__input'), {mask: "+{7}(000)000-00-00", lazy: false});
			}
			contactsArray.push({
				type: element.querySelector('.contact-select').getAttribute('data-contact'),
				value: element.querySelector('.contact-select').dataset.contact === 'phone' ? mask.unmaskedValue : element.querySelector('.contact-form__input').value});
		})
	...
	function validateInput({fieldName = 'Контактов', inputField}, {errorWrapper, errorFlag}) {
		...
		inputField.addEventListener('input', () => {
			...
			if (inputField.value.trim() && inputField.parentNode.dataset.field !== 'optional') {
				errorRemoval({fieldName, inputField}, errorContent);
			}
		})
		...
		if (typeof fieldName !== 'object') {
			const contactType = inputField.closest('.contact-form__field').querySelector('[data-contact]').dataset.contact
			switch (contactType) {
				case 'phone': {
	...
	function createSvgIcon({
		...
		if (appendInto) {
			return insertType ? appendInto.appendChild(svgNsMap.get('svg')) : appendInto.prepend(svgNsMap.get('svg'));
		} else {
	...
	document.addEventListener('DOMContentLoaded', async () => {
		...
		const eventHandlers = {
			async onSave(clientInfo, clientData) {
				if (clientData.id) {
					await fetch(`${URL_PATH}${clientData.id}`, {
						method: 'PATCH',
			...
			const rotateIcon = () => {
				...
				tableHead.buttons.forEach(btn => {
					if (btn !== this) {
						...
						btn.querySelector('.clients-table__ico').classList.remove('active-sort-ico');
				...
				this.querySelector('.clients-table__ico').classList.remove('active-sort-ico');
